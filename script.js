...
const API_KEY = "7a15c2da448f4bf6a7090715241109";

//Wather codes for mapping to custom icons
const weatherCodes = {
  clear: [1000],
  ...
  moderate_heavy_rain: [1186, 1189, 1192, 115, 1243, 1246],
  snow: [
  ...
  hourlyWeatherDiv.innerHTML = next24HoursData
    .map((item) => {
      ...
      return ` <li class="weather-item">
              <p class="time">${time}</p>
              <img src="icons/${weatherIcon}.svg" alt="error" class="weather-icon" />
              <p class="temprature">${temprature}°</p>
            </li> `;
    })
    .join("");
};
...
const getWeatherDetails = async (API_URL) => {
  ...
  try {
    ...
    currentWeatherDiv.querySelector(
      ".temprature"
    ).innerHTML = `${temprature}<span>°C</span>`;
...
// Handle uesr input in the search box
